...
export const drawChart = (
  ...
  let width =
    parseInt(d3.select("#chart").style("width")) - margins.left - margins.right;
  ...
  dataGroup.forEach((d, i) => {
    ...
    svg
      ...
      .attr(
        "transform",
        "translate(" +
          xScale(d.values[d.values.length - 1][objX]) +
          "," +
          yScale(d.values[d.values.length - 1][objY]) +
          ")"
      )
  ...
  function resize() {
    ...
    dataGroup.forEach((d, i) => {
      svg.select(".line" + i).attr("d", lineGen(d.values));
      svg
        .select(".textKey" + i)
        .attr(
          "transform",
